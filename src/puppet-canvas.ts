  ...
  return async (request: Op): Promise<any> => {
    try {
        ...
        async (
          ...
          const derefArg = (arg: any) => {
            if (
              arg &&
              typeof arg === "object" &&
              (arg as DeferredReference).type === "_deferred_"
            ) {
          ...
          const reducePath = (list: PropName[]) =>
            list.reduce<any>((o: any, prop) => (o ? o[prop] : o), jsTarget);
          ...
          if (typeof out === "object") {
            ...
            const refMap = cw.$puppetCanvasMap.get(canvasElement);
            if (refMap) {
              const refId = `${Date.now()}-${Math.floor(
              ...
              throw new Error(
                "Canvas element not initialized as puppet-canvas"
              );
            }
          }
          return out;
      ...
      if (
        ...
        (result as DeferredReference).type === "_deferred_"
      ) {
        ...
        return deferredHandle
          ? proxy(createHandler(canvasHandle, deferredHandle), [], refId)
          : null;
      }
      return result;
    } catch (err) {
...
async function getHandlyByRefId(
  canvasHandle: ElementHandle<HTMLCanvasElement>,
  refId: string
): Promise<JSHandle> {
  return await canvasHandle.evaluateHandle(
    (canvasElement: HTMLCanvasElement, refId: string) => {
      const cw = self as any as CanvasWindow;
      const refMap = cw.$puppetCanvasMap.get(canvasElement);
      if (refMap) {
...
export async function linkCanvas(
...
export async function createCanvas(
  ...
  const canvasElement: ElementHandle<HTMLCanvasElement> | null = await page.$(
    "canvas"
  );
...
export async function loadFont(
  ...
  if (cachedRecord) {
    await cachedRecord.canvasHandle.evaluate(
      async (_, fontName: string, fontUrl: string) => {
...
export async function loadImage(
  ...
  if (pageToUse) {
    ...
    const imageRef = await pageToUse.evaluate(
      (canvasElement: HTMLCanvasElement, url: string) => {
